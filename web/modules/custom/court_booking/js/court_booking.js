...
(function (Drupal, drupalSettings, once) {
  ...
  /**
   * Display name for court cards: linked court node title from PHP, else variation title.
   *
   * @param {object} v
   * @returns {string}
   */
  function variationCardTitle(v) {
    if (!v) {
      return '';
    }
    const court = v.courtTitle != null && String(v.courtTitle).trim() !== '' ? String(v.courtTitle).trim() : '';
    if (court) {
      return court;
    }
    return v.title != null ? String(v.title) : '';
  }
      ...
      once('court-booking', '.court-booking-page', context).forEach((root) => {
        ...
        function renderCourtsOverview() {
          ...
          list.forEach((v) => {
            ...
            h.textContent = variationCardTitle(v);
        ...
        function renderCourts(startIso) {
          ...
          if (bufferMinutes > 0 && s.slotCandidatesUrl && bufferSlotCandidates && bufferSlotCandidates.length) {
            ...
            blocks.forEach(({ v, block }) => {
              ...
              card.setAttribute('aria-label', Drupal.t('Book @title', { '@title': variationCardTitle(v) }));
